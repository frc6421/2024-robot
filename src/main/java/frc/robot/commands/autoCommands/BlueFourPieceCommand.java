...
public class BlueFourPieceCommand extends SequentialCommandGroup {
  ...
  public BlueFourPieceCommand(DriveSubsystem drive, IntakeSubsystem intake, TransitionSubsystem transition, ShooterSubsystem shooter, ShooterAngleSubsystem shooterAngle) {
    ...
    addCommands(
        new InstantCommand(() -> driveSubsystem.seedFieldRelative(driveToFirstNoteTrajectory.getInitialPose()), driveSubsystem), 
        // shoot preload
        new InstantCommand(() -> shooterAngleSubsystem.getTargetAngle(), shooterAngleSubsystem),
        new ShooterRevUpCommand(shooterSubsystem, 5400),
        new InstantCommand(() -> transitionSubsystem.setTransitionVoltage(TransitionConstants.TRANSITION_SPEED), transitionSubsystem),
        new WaitCommand(0.2),
        new InstantCommand(() -> transitionSubsystem.stopTransition(), transitionSubsystem),
        new InstantCommand(() -> shooterSubsystem.stopShooterMotor(), shooterSubsystem),
        ...
        new InstantCommand(() -> shooterAngleSubsystem.getTargetAngle()),
        new ShooterRevUpCommand(shooterSubsystem, 5400),
        new InstantCommand(() -> transitionSubsystem.setTransitionVoltage(TransitionConstants.TRANSITION_SPEED), transitionSubsystem),
        new WaitCommand(0.2),
        new InstantCommand(() -> transitionSubsystem.stopTransition(), transitionSubsystem),
